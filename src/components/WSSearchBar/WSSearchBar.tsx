...
const WSSearchBar: FC<CustomSearchProps> = ({ placeholder, defaultResult=null, onSearch, onSelect, label, intermediate=false, ...props }) => {
    ...
    return (
        <div className={`${styles.WSSearchBar}`}>
            <WSInput name='searchInput' intermediate={intermediate} onClick={handleClick} autoComplete='off' id='searchInput' placeholder={placeholder} label={label} value={query} change={() => {}} onChange={handleInputChange} onBlur={handleInputBlur} onKeyDown={handleInputKeyDown}  {...props} />
            {showResults && (
